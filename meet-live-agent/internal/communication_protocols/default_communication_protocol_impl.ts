  ...
  implements MediaApiCommunicationProtocol
{
  ...
  async connectActiveConference(
    ...
    // Call to Meet API
    const spaceId = this.requiredConfiguration.meetingSpaceId.startsWith('spaces/') ? this.requiredConfiguration.meetingSpaceId : `spaces/${this.requiredConfiguration.meetingSpaceId}`;
    const connectUrl = `${this.meetApiUrl}${spaceId}:connectActiveConference`;
    console.log('[DefaultCommunicationProtocolImpl] SDP Offer:', sdpOffer);
